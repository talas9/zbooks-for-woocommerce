	...
	public function run(): void {
		...
		foreach ( $failed_orders as $order ) {
			...
			if ( ! $this->can_retry_order( $order, $settings ) ) {
				$this->logger->debug(
					'Order retry skipped: max retries reached',
					[
						'order_id'    => $order_id,
			...
			if ( ! $this->should_retry_now( $order ) ) {
				$this->logger->debug(
					'Order retry skipped: backoff delay not elapsed',
					[
						'order_id' => $order_id,
					]
				);
