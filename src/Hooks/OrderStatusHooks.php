...
add_action('zbooks_sync_order', function (int $order_id, bool $as_draft) {
    $plugin = \Zbooks\Plugin::get_instance();
    $hooks = $plugin->get_service('order_status_hooks');
    $hooks->execute_sync($order_id, $as_draft);
}, 10, 2);

add_action('zbooks_apply_payment', function (int $order_id) {
    $plugin = \Zbooks\Plugin::get_instance();
    $hooks = $plugin->get_service('order_status_hooks');
    $hooks->execute_apply_payment($order_id);
}, 10, 1);

add_action('zbooks_process_refund', function (int $order_id, int $refund_id) {
    $plugin = \Zbooks\Plugin::get_instance();
    $hooks = $plugin->get_service('order_status_hooks');
