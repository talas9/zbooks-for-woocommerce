...
class InvoiceService {
    ...
    public function find_invoice_by_reference(string $reference): ?string {
        try {
            $invoices = $this->client->request(function ($client) use ($reference) {
            ...
            $this->logger->warning('Failed to search for invoice by reference', [
                'reference' => $reference,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    public function find_invoice_by_invoice_number(string $invoice_number): ?string {
        try {
            $invoices = $this->client->request(function ($client) use ($invoice_number) {
                return $client->invoices->getList([
            ...
            $this->logger->warning('Failed to search for invoice by invoice number', [
                'invoice_number' => $invoice_number,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    public function mark_as_sent(string $invoice_id): bool {
        try {
            ...
            $this->logger->warning('Failed to mark invoice as sent', [
                'invoice_id' => $invoice_id,
                'error' => $e->getMessage(),
            ]);
            return false;
    ...
    private function map_order_to_invoice(WC_Order $order, string $contact_id): array {
        ...
        if (!empty($custom_fields)) {
            ...
            $this->logger->debug('Adding custom fields to invoice', [
                'order_id' => $order->get_id(),
        ...
        foreach ($order->get_items() as $item) {
            ...
            if ($product) {
                ...
                if (!empty($zoho_item_id)) {
                    ...
                    $this->logger->debug('Using mapped Zoho item', [
