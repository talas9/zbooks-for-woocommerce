...
class RefundService {
	...
	public function process_refund(
		...
		try {
			...
			if ( ! empty( $refund_settings['create_cash_refund'] ) ) {
				...
				$this->logger->debug(
					'Cash refund creation skipped (disabled in settings)',
					[
	...
	private function create_credit_note(
		...
		try {
			$response = $this->client->request(
				function ( $client ) use ( $credit_note_data ) {
	...
	private function apply_credit_to_invoice(
		...
		if ( ! $status_check['can_apply'] ) {
			$this->logger->warning(
				'Cannot apply credit note due to invoice status',
				[
					'credit_note_id' => $credit_note_id,
					'invoice_id'     => $invoice_id,
		...
		try {
			...
			$this->logger->debug(
				'Credit note applied to invoice via invoice endpoint',
				[
					'credit_note_id' => $credit_note_id,
					'invoice_id'     => $invoice_id,
					'amount'         => $amount,
			...
			$this->logger->debug(
				'Invoice endpoint failed, trying creditnote endpoint',
				[
					'error' => $e->getMessage(),
				]
			);
		...
		try {
			$this->client->raw_request(
				'POST',
				'/creditnotes/' . $credit_note_id . '/invoices',
				[
					'invoices' => [
						[
							'invoice_id'     => $invoice_id,
			...
			$this->logger->debug(
				'Credit note applied to invoice via creditnote endpoint',
				[
					'credit_note_id' => $credit_note_id,
					'invoice_id'     => $invoice_id,
					'amount'         => $amount,
			...
			if ( strpos( $error_msg, 'not authorized' ) !== false ) {
				$this->logger->warning(
					'Credit note application requires additional OAuth scopes',
					[
						'credit_note_id'  => $credit_note_id,
						'invoice_id'      => $invoice_id,
				...
				$this->logger->warning(
					'Cannot apply credit to draft invoice',
					[
						'invoice_id' => $invoice_id,
					]
				);
				...
				$this->logger->warning(
					'Failed to apply credit note to invoice',
					[
						'credit_note_id' => $credit_note_id,
						'invoice_id'     => $invoice_id,
						'error'          => $error_msg,
	...
	private function create_refund_from_credit(
		...
		try {
			...
			if ( empty( $account_id ) ) {
				$this->logger->debug(
					'No refund account configured, skipping refund record creation',
					[
						'credit_note_id' => $credit_note_id,
					]
				);
			...
			$this->logger->warning(
				'Failed to create credit note refund',
				[
					'credit_note_id' => $credit_note_id,
					'error'          => $e->getMessage(),
				]
	...
	public function get_credit_note( string $credit_note_id ): ?array {
		try {
			$response = $this->client->request(
				function ( $client ) use ( $credit_note_id ) {
