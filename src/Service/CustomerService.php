...
class CustomerService {
    ...
    public function find_or_create_contact(WC_Order $order): string {
        ...
        if ($existing_contact !== null) {
            ...
            $this->logger->debug('Found existing contact', [
                'email' => $email,
    ...
    public function check_currency_compatibility(array $contact, WC_Order $order): array {
        ...
        if (!$compatible) {
            $this->logger->warning('Currency mismatch detected', [
                'email' => $order->get_billing_email(),
    ...
    public function get_contact_by_email(string $email): ?array {
        try {
            $response = $this->client->request(function ($client) use ($email) {
            ...
            $this->logger->warning('Failed to search for contact', [
                'email' => $email,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function map_order_to_contact(WC_Order $order): array {
        ...
        if (!empty($custom_fields)) {
            ...
            $this->logger->debug('Adding custom fields to contact', [
                'email' => $order->get_billing_email(),
    ...
    public function update_contact(string $contact_id, WC_Order $order): bool {
        ...
        try {
            ...
            $this->logger->warning('Failed to update contact', [
                'contact_id' => $contact_id,
                'error' => $e->getMessage(),
            ]);
            return false;
    ...
    private function contact_needs_update(array $contact, WC_Order $order): bool {
        ...
        if ($order_name !== $contact_name) {
            $this->logger->debug('Contact name changed', [
                'old' => $contact_name,
                'new' => $order_name,
            ]);
            return true;
