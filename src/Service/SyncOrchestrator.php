...
class SyncOrchestrator {
    ...
    public function detect_conflicts(WC_Order $order): array {
        ...
        if ($existing_invoice_id !== null) {
            ...
            $this->logger->info('Conflict detected: Invoice already exists in Zoho', [
                'order_id' => $order->get_id(),
        ...
        if (!empty($email)) {
            ...
            if ($existing_contact !== null) {
                ...
                $this->logger->debug('Existing contact found in Zoho', [
                    'email' => $email,
                ...
                if (!$currency_check['compatible']) {
                    ...
                    $this->logger->warning('Conflict detected: Currency mismatch', [
                        'order_id' => $order->get_id(),
                        'email' => $email,
    ...
    public function sync_order_with_conflict_check(WC_Order $order, bool $as_draft = false): SyncResult {
        ...
        if ($conflicts['has_conflict'] && $conflicts['invoice_id'] !== null) {
            ...
            $this->logger->info('Linking existing Zoho invoice to order', [
                'order_id' => $order_id,
            ...
            return SyncResult::success(
                ...
                contact_id: $contact_id,
                status: SyncStatus::SYNCED,
                data: ['conflict_resolved' => true, 'linked_existing' => true]
            );
        }
    ...
    public function apply_payment(WC_Order $order): array {
        ...
        if (!$this->payment_service) {
            $this->logger->warning('Payment service not available', [
                'order_id' => $order_id,
            ]);
            return [
                'success' => false,
        ...
        if (!$invoice_id) {
            $this->logger->warning('Cannot apply payment - order not synced', [
                'order_id' => $order_id,
            ]);
            return [
                'success' => false,
        ...
        if ($this->repository->has_payment($order)) {
            ...
            $this->logger->debug('Payment already recorded', [
                'order_id' => $order_id,
    ...
    public function process_refund(WC_Order $order, WC_Order_Refund $refund): array {
        ...
        if (!$this->refund_service) {
            $this->logger->warning('Refund service not available', [
                'order_id' => $order_id,
                'refund_id' => $refund_id,
            ]);
            return [
        ...
        $invoice_id = $this->repository->get_invoice_id($order);
        if (!$invoice_id) {
            $this->logger->warning('Cannot process refund - order not synced', [
                'order_id' => $order_id,
                'refund_id' => $refund_id,
            ]);
            return [
